...
class UDPTrackerClient:
    ...
    async def _initiate_connection(self, tid: int) -> bytes:
        ...
        transport, protocol = await self.tracker_send_and_receive(
            protocol, timeout=get_config().services.tracker_scraping.connection_timeout
        )
...
class HTTPTrackerClient:
    ...
    def __init__(
        ...
        url: str,
        max_scrape: int = MAX_SCRAPE,
        timeout: int = get_config().services.tracker_scraping.connection_timeout,
    ):
    ...
    async def scrape(self, infohashes: list[str]) -> ScrapeResult:
        ...
        async with httpx.AsyncClient() as client:
            if self.announce_url in get_config().services.tracker_scraping.http_tracker_scrape_all:
    ...
    async def _scrape_page(
        ...
        try:
            ...
            if (
                ...
                and self.announce_url
                in get_config().services.tracker_scraping.http_tracker_single_only
            ):
...
async def _scrape_udp_tracker(url: str, infohashes: list[str], logger: Logger) -> ScrapeResult:
    ...
    try:
        client = await UDPTrackerClient.from_url(
            url, timeout=get_config().services.tracker_scraping.scrape_timeout
        )
...
def _handle_tracker_error(results: Sequence[ScrapeResult], logger: Logger) -> None:
    ...
    tracker_errors = [
        e for tracker in results for e in tracker.errors if e.announce_url and e.infohash is None
    ]
...
def _compute_backoff(
    ...
    multipliers = get_config().services.tracker_scraping.backoff.model_dump()
...
def _touch_tracker(results: Sequence[ScrapeResult]) -> None:
    from sciop.db import get_session
    from sciop.models import Tracker

    params = []
    for tracker in results:
        if not tracker.errors and not tracker.responses:
            continue
        announce_url = (
            tracker.errors[0].announce_url
            if tracker.errors
            else next(iter(tracker.responses.values())).announce_url
        )

        item = {"last_scraped_at": datetime.now(UTC), "b_announce_url": announce_url}
        if not tracker.errors:
            item.update({"n_errors": 0, "error_type": None, "next_scrape_after": None})
        params.append(item)
