...
class LoggingMiddleware(BaseHTTPMiddleware):
    ...
    def _log_message(
        ...
        time_msg = ""
        if config.logs.request_timing and request_total_time is not None:
...
async def security_headers(request: Request, call_next: RequestResponseEndpoint) -> Response:
    ...
    """
    nonce = token_urlsafe(config.server.csp.nonce_entropy)
    ...
    sec_headers = {
        "Content-Security-Policy": config.server.csp.format(nonce),
    ...
    if "/docs" in request.url.path:
        sec_headers["Content-Security-Policy"] = (
            type(config.server.csp)
            .model_construct(
                **{
                    **config.server.csp.model_dump(),
